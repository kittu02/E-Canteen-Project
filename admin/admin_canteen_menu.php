...
<body>
    ...
    <div id="canteen_menu" class="m-4">
      ...
      ?>
        <tr>
          ...
          <td>
              <button class="btn btn-warning" type="submit" name="btnEdit" onclick="toggleFloatingDiv()"value="<?php echo $row['food_id']; ?>">
                    <span><i class="fa fa-pencil" aria-hidden="true"></i></span>
                </button>
              <button class="btn btn-danger" type="submit" name="deleteNow" onclick="toggleFloatingDiv()"value="<?php echo $row['food_id']; ?>">
  ...
  ?>
    ...
    <div id="floatingDiv" class="editMenu">
        <form method="post" action="admin_canteen_menu.php" class="floatingContent">
            ID 
            <input type="text" name="txtID"><?php $row['food_id'] ?><br/>
            Name 
            <input type="text" name="txtName"><?php $row['food_name'] ?><br/>
            Photo Link 
            <input type="text" name="txtPhoto"><?php $row['food_photo'] ?> <br/>
            Price 
            <input type="text" name="txtPrice"><?php $row['food_pric'] ?> <br/>
            Description 
            <input type="text" name="txtDesc" ><?php $row['food_dec'] ?><br/>
            Ingredients 
            <input type="text" name="txtIngred" ><?php $row['food_ingred'] ?><br/>
            Expected Time 
            <input type="text" name="time" > <?php $row['time'] ?><br/>
            Quantity 
            <input type="text" name="quantity" > <?php $row['quantity'] ?><br/>
            <button type="submit" name="editNow" onclick="closeFloatingDiv()">Edit to List</button>  
        </form>   
    </div>
