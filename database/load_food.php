...
$sqlOrderTable = "CREATE TABLE IF NOT EXISTS list_order(order_id VARCHAR(10) NOT NULL,order_no VARCHAR(15) NOT NULL , enrolment_no INT(15) NOT NULL , canteen_number INT(2) NOT NULL , food_id VARCHAR(6) NOT NULL , quantity INT(10) NOT NULL, payment_status INT(1) NOT NULL , order_date DATE NOT NULL , order_time TIME NOT NULL, order_status INT(1) NOT NULL, check_time TIME, PRIMARY KEY(order_id), FOREIGN KEY(food_id) REFERENCES food_list(food_id), FOREIGN KEY(enrolment_no) REFERENCES user_table(enrolment_no));";
